...
- 学べる内容（3モード、共通の敵8体で戦う）
  1. **かけ算モード** — 九九の計算問題（数字入力・約60%）とかけ算文章題（式の3択・約40%）をまぜて出題
  2. **漢字モード** — 小学1・2年生で習う漢字約56語を、読み・文章の中の読み・反対語・なかま・並び（季節/曜日/方角）などの形式で出題
  3. **まぜまぜモード** — 九九の計算問題と漢字の問題をだいたい半分ずつまぜて出題
...
- ✅ まぜまぜモード：実際に遊べる。九九の計算問題と漢字の問題をだいたい半分ずつまぜて出題（`js/mixedMode.js`）。既存の`generateKukuQuestion(tier)`と`generateKanjiQuestionForTier(tier)`をそのまま再利用しているだけなので、回答UI（数字入力/3択）・敵の強さによる難易度調整もかけ算モード・漢字モードとまったく同じ挙動になる
- ✅ 敵の強さに応じた難易度調整：かけ算モード・漢字モード・まぜまぜモードいずれも、今戦っているモンスターの`tier`（zako/boss/lastboss）に応じて出題の傾向が少しずつ難しくなる。ザコ敵はやさしめ、中ボスはやや難しめ、ラスボスは中ボスと同じ傾向をより強めに（詳細は下記6.）
...
- ✅ バトル画面左上の🏠ボタン（`#btn-battle-home`）で、途中でもタイトルにもどれる。演出中（キック演出やBGM切り替えの`setTimeout`/`await sleep`チェーンの途中）に押しても、`battleToken`という世代カウンターで古い演出を無効化しているため、タイトル画面に戻ったあとに古い演出が遅れて反映されることはない（`js/main.js` の `startBattle`/`playCorrectSequence`/`playWrongSequence` を参照）
...
│   ├── kanjiMode.js           # 「漢字モード」：読み・反対語・なかま・並びなどを出題する
│   ├── mixedMode.js           # 「まぜまぜモード」：九九と漢字を半々に混ぜて出題する
...
`js/battle.js` の `askNextQuestion()` が `mode.nextQuestion(currentMonster().tier)` のようにtierを渡し、各モードが出題の傾向を変える。かけ算モード・漢字モードとも、モード選択自体やモード内の基本比率（九九:文章題=60:40など）は変えていない。まぜまぜモード（`js/mixedMode.js`）は九九:漢字=50:50で`generateKukuQuestion(tier)`/`generateKanjiQuestionForTier(tier)`を直接呼んでいるだけなので、難易度調整ロジック自体は増やしていない（既存分をそのまま共有）。
